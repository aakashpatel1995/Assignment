...
try{
    const regex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
 const {firstname,lastname,email,password,role,userstatus,industry}=req.body;
 if (!firstname || !lastname || !email || !password || !role || !industry) {
...
const dateofRegister = new Date(); 
 ...
 const user=new User({firstname,lastname,email,password:passowrdHashed,role,industry,userstatus,dateofRegister});
 await user.save();
